...
func SetQueryConds(conds *npool.Conds, cli *ent.Client) (*ent.ContactQuery, error) {
	...
	if conds.AccountType != nil {
		...
		case cruder.EQ:
			stm.Where(contact.AccountType(basetypes.SignMethodType(conds.GetAccountType().GetValue()).String()))
		default:
			return nil, fmt.Errorf("invalid contact field")
		}
	}
	...
	if conds.UsedFor != nil {
		...
		case cruder.EQ:
			stm.Where(contact.UsedFor(basetypes.UsedFor(conds.GetUsedFor().GetValue()).String()))
		default:
			return nil, fmt.Errorf("invalid contact field")
		}
	}
