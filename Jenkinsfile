  ...
  stages {
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          images=`docker images | grep entropypool | grep notif-manager | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep notif-manager | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/notif-manager/k8s/02-notif-manager.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/notif-manager:latest/notif-manager:$tag/g" cmd/notif-manager/k8s/02-notif-manager.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/notif-manager/k8s/02-notif-manager.yaml
